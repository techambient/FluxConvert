...
set "ASSETS_DIR=C:\fluxconvert-assets"
...
if !HAS_ITEMS! LSS 8 (
    ...
    mkdir "%TEMP%\fluxconvert_temp"
    
    echo [INFO] Downloading repository...
    powershell -Command "Invoke-WebRequest -Uri 'https://github.com/techambient/FluxConvert/archive/refs/heads/main.zip' -OutFile '%TEMP%\fluxconvert_temp\main.zip'"
    
    echo [INFO] Extracting files...
    powershell -Command "Expand-Archive -Path '%TEMP%\fluxconvert_temp\main.zip' -DestinationPath '%TEMP%\fluxconvert_temp\extracted' -Force"
    
    :: Create target assets directory on C:\
    mkdir "%ASSETS_DIR%"
    
    :: Move contents from the nested FluxConvert-main folder into C:\fluxconvert-assets
    xcopy /s /e /y "%TEMP%\fluxconvert_temp\extracted\FluxConvert-main\*.*" "%ASSETS_DIR%\" >nul
    
    :: Clean up temp files
    rmdir /s /q "%TEMP%\fluxconvert_temp"
